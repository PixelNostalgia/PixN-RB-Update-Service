...
powershell.exe -NoProfile -NonInteractive -ExecutionPolicy Bypass -Command ^
    ...
    "    [xml]$xml = Get-Content -LiteralPath $path; " ^
    "    if ($xml.config -eq $null) { throw 'Root <config> node not found' } " ^
    "    @($xml.config.string | Where-Object { $_.name -eq 'win98.dosbox_pure_bootos_ramdisk' }) | ForEach-Object { [void]$xml.config.RemoveChild($_) }; " ^
    "    $newNode = $xml.CreateElement('string'); " ^
    "    [void]$newNode.SetAttribute('name','win98.dosbox_pure_bootos_ramdisk'); " ^
    "    [void]$newNode.SetAttribute('value','true'); " ^
    "    [void]$xml.config.AppendChild($newNode); " ^
    "    $xml.Save($path); " ^
    "} catch { " ^
